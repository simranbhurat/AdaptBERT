# -*- coding: utf-8 -*-
"""AdaptBERT.ipynb is located at
    https://colab.research.google.com/drive/10hzTHUss9x8lSD_J14U-6jHad2Gk9KUb
"""
...
#Read patent data
patents = pd.read_csv('final_patent.csv')
patents = patents.dropna()
patents = patents.reset_index(drop=True)
possible_labels = patents['cpc_code'].unique()
numerical_encoding_dict = {}
for index, possible_label in enumerate(possible_labels):
    numerical_encoding_dict[possible_label] = index
patents['cpc_class_numerical'] = patents['cpc_code'].replace(numerical_encoding_dict)
patents['text'] = patents['patent_title'].astype(str) + ' ' + patents['patent_abstract'].astype(str)
patents['text'] = patents['text'].apply(lambda x: x.lower())
patents['text'] = patents['text'].apply(clean_abstract)
# metadata: review year ids
patents['year'] = pd.to_datetime(patents['patent_date']).dt.year
year_dict = {k: k-patents['year'].min() for k in patents['year'].unique()}
patents['year_ids'] = patents['year'].replace(year_dict)
patents.reset_index(inplace = True)
patents = patents[['patent_id', 'cpc_code', 'cpc_class_numerical', 'text', 'year', 'year_ids']]
result = patents.groupby(['cpc_code', 'year']).apply(lambda x: x.sample(n=min(50, len(x)), replace=False) if len(x) > 50 else x)
# result = patents.groupby(['cpc_code', 'year']).head(50)
patents = result.reset_index(drop=True)

#Get Median
token_lengths = patents['text'].apply(lambda x: len(x.split()))
...
rd = rd.reset_index(drop=True)
rd = rd.iloc[:patents.shape[0]]

# tranform data to lists
feature_text_rd, feature_year_rd = rd['text_all'].to_list(), rd['year_ids'].to_list()
...
# training size
train_size = 0.6
training_data, test_data = sklearn.model_selection.train_test_split(patents, test_size=test_size, random_state=parameter_dict['seed'])
...
class BertClassificationMetadata(transformers.BertPreTrainedModel):
    def __init__(self, config, num_year_ids, metadata_embedding_size, hidden_layer_size, dropout_fine_tune):
        """
        Initialize the BertClassificationMetadata model.

        Parameters:
            config (BertConfig): The configuration of the BERT model.
            num_year_ids (int): The number of unique year IDs for metadata.
            metadata_embedding_size (int): The size of the metadata embedding.
            hidden_layer_size (int): The size of the hidden layer for the classification head.
            dropout_fine_tune (float): The dropout probability for fine-tuning.
    ...
    def forward(
        ...
        """
        """
        Perform a forward pass through the BertClassificationMetadata model.

        Parameters:
            input_ids (torch.Tensor): Input token IDs.
            attention_mask (torch.Tensor): Attention mask to avoid performing attention on padding tokens.
            ...
            inputs_embeds (torch.Tensor): Instead of passing input_ids, directly pass the input embeddings.
            labels (torch.Tensor): Labels for computing the sequence classification/regression loss.
            output_attentions (bool): Whether to output attentions weights.
            output_hidden_states (bool): Whether to output all hidden states.
            return_dict (bool): Whether to return a dictionary or a data class instance.
            year_ids (torch.Tensor): Metadata year IDs for the input samples.

        Returns:
            transformers.modeling_outputs.SequenceClassifierOutput:
                loss (torch.Tensor, optional): The classification loss (CrossEntropy) if labels are provided, else None.
                logits (torch.Tensor): The logits (raw scores) for each class.
                hidden_states (Tuple[torch.Tensor]): Tuple of all hidden states at the output of the last layer.
                attentions (Tuple[torch.Tensor]): Tuple of attention weights for each layer.
...
feature_text_target = fast_encode(feature_text_rd, tokenizer)
...
target_torch = AttributeDataset(feature_text_target, feature_year_rd)
...
bert_classification_model = torch.load((f'{model_name}.pt'), map_location=torch.device(device))
...
def sector_wise_accuracy(confusion_matrix_df):
    sector_wise_accuracy_dict = {}
    for class_label in confusion_matrix_df.index:
        ...
        sector_wise_accuracy_dict[class_label] = class_accuracy

    return sector_wise_accuracy_dict
...
# Calculate Sector-wise accuracy using the function
class_wise_acc = sector_wise_accuracy(cm_df_sorted)

# Print the Sector-wise accuracy
print("Sector-wise Accuracy:")
...
# plt.savefig((f'{model_name}/confusion_matrix_{model_name}_test.png'), format='png', dpi=300, bbox_inches='tight')
...
rd_predicted.to_csv(f'rd_prediction_{model_name}.csv')
